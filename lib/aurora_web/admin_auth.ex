defmodule AuroraWeb.AdminAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_aurora_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      AuroraWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_admin:
      ...
      defmodule AuroraWeb.PageLive do
        use AuroraWeb, :live_view

        on_mount {AuroraWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{AuroraWeb.AdminAuth, :ensure_authenticated}] do
  ...
  def on_mount(:require_authenticated_admin, _params, session, socket) do
    ...
    if socket.assigns.current_admin do
      ...
      {:halt, Phoenix.LiveView.redirect(socket, to: ~p"/log_in")}
    end
  end
