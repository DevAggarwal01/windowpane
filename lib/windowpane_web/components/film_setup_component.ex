defmodule WindowpaneWeb.FilmSetupComponent do
  ...
  def handle_event("save", %{"project" => project_params}, socket) do
    Logger.warning("SAVE: Project ID: #{socket.assigns.project.id}, Params: #{inspect(project_params)}")
    case Projects.update_project(socket.assigns.project, project_params) do
      {:ok, project} ->
        ...
        Logger.warning("SAVE: Project updated successfully")
        {:noreply,
         socket
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        Logger.warning("SAVE: Project update failed: #{inspect(changeset.errors)}")
  ...
  def handle_event("cancel_edit", _, socket) do
    Logger.warning("CANCEL_EDIT: Project ID: #{socket.assigns.project.id}")
    send(self(), {:project_updated, socket.assigns.project, false})
    {:noreply,
     socket
  ...
  def handle_event("deploy", _, socket) do
    ...
    Logger.warning("DEPLOY: Starting deployment for Project ID: #{project.id}")
    ...
    if Projects.ready_for_deployment?(project) do
      Logger.warning("DEPLOY: Project is ready for deployment")
      case Projects.add_to_approval_queue(project) do
        {:ok, _queue_entry} ->
          ...
          Logger.warning("DEPLOY: Adding project to approval queue")
          case Projects.update_project(project, %{status: "waiting for approval"}) do
            {:ok, updated_project} ->
              ...
              Logger.warning("DEPLOY: Project successfully deployed and status updated")
              {:noreply,
               socket
      ...
      Logger.warning("DEPLOY: Project not ready for deployment")
      {:noreply,
       socket
  ...
  def handle_event("open_cover_modal", _params, socket) do
    Logger.warning("OPEN_COVER_MODAL: Project ID: #{socket.assigns.project.id}")
  ...
  def handle_event("upload_error", %{"error" => error}, socket) do
    Logger.error("UPLOAD_ERROR: Project ID: #{socket.assigns.project.id}, Error: #{error}")
    {:noreply,
     socket
     |> assign(:cover_uploading, false)
  ...
  def handle_event("open_banner_modal", _params, socket) do
    Logger.warning("OPEN_BANNER_MODAL: Project ID: #{socket.assigns.project.id}")
  ...
  def handle_event("banner_upload_error", %{"error" => error}, socket) do
    Logger.error("BANNER_UPLOAD_ERROR: Project ID: #{socket.assigns.project.id}, Error: #{error}")
    {:noreply,
     socket
     |> assign(:banner_uploading, false)
  ...
  def render(assigns) do
    ...
    <div>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
              ...
              >
                ...
                <div class="flex gap-4">
                  ...
                  <.button type="button" phx-click="cancel_edit" phx-target={@myself} class="bg-gray-500 hover:bg-gray-600">
                    Cancel
                  </.button>
                </div>
              </.form>
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="mt-4" id="image-cropper-hook" phx-hook="ImageCropper" data-project-id={@project.id} phx-target={@myself}>
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="mt-4" id="banner-cropper-hook" phx-hook="BannerCropper" data-project-id={@project.id} phx-target={@myself}>
              <!-- Include Cropper.js CDN -->
              <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.1/cropper.min.css">
              <script src="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.1/cropper.min.js"></script>
      ...
      <!-- Cover Image Modal -->
      <.modal :if={@show_cover_modal} id="cover-image-modal" show on_cancel={JS.push("close_cover_modal", target: @myself)}>
      ...
      <!-- Cropper Modal -->
      <.modal :if={@show_cropper_modal} id="cropper-modal" show on_cancel={JS.push("hide_cropper_modal", target: @myself)}>
        <div class="text-center">
          <div class="mb-4">
      ...
      <!-- Banner Image Modal -->
      <.modal :if={@show_banner_modal} id="banner-image-modal" show on_cancel={JS.push("close_banner_modal", target: @myself)}>
        <div class="text-center">
          <div class="mb-4">
      ...
      <.modal :if={@show_banner_cropper_modal} id="banner-cropper-modal" show on_cancel={JS.push("hide_banner_cropper_modal", target: @myself)}>
        <div class="text-center">
          <div class="mb-4">
