defmodule Windowpane.Creators.Creator do
  use Ecto.Schema
  ...
  @doc """
  A creator changeset for registration.
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique(:email, Windowpane.Repo)
  ...
  @doc """
  A creator changeset for changing the email.

  It requires the email to change otherwise an error is added.
  """
  def email_changeset(creator, attrs, opts \\ []) do
    creator
    |> cast(attrs, [:email])
    |> validate_email(opts)
    |> case do
  ...
  @doc """
  A creator changeset for changing the password.
  ...
  ## Options
    ...
    * `:hash_password` - Hashes the password so it can be stored securely
      ...
      Defaults to `true`.
  """
  def password_changeset(creator, attrs, opts \\ []) do
    creator
    |> cast(attrs, [:password])
  ...
  """
  def confirm_changeset(creator) do
    now = DateTime.utc_now() |> DateTime.truncate(:second)
    change(creator, confirmed_at: now)
  end

  @doc """
  Verifies the password.

  If there is no creator or the creator doesn't have a password, we call
  `Bcrypt.no_user_verify/0` to avoid timing attacks.
  """
  def valid_password?(%Windowpane.Creators.Creator{hashed_password: hashed_password}, password)
